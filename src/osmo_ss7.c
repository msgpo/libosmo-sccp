...
/* ::ffff:172.18.19.200 => 172.18.19.200 */
static void chop_v4_mapped_on_v6_prefix(char* buf)
{
	...
	if (!last_colon)
		return; /* full IPv4 address, nothing to do */
...
osmo_ss7_asp_find_by_socket_addr(int fd)
{
	...
	LOGP(DLSS7, LOGL_NOTICE, "Looking up ASP for fd=%d\n", fd);
	...
	/* If multi-home is used with both IPv4 and IPv6, then the socket is
	   AF_INET6, and then returned IPv4addresses are actually v6mapped ones.
	...
	LOGP(DLSS7, LOGL_NOTICE, "Looking up ASP for l=%s<->r=%s\n", hostbuf_l, hostbuf_r);
